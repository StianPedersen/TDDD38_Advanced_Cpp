...
namespace ns
{
    template <typename C, typename F> // 3
    auto enumerate_helper(C const &c, F f, int, int, int)
        -> decltype(f(*std::begin(c), 0, 0), std::declval<void>())
    ...
    template <typename C, typename F> // 2
    auto enumerate_helper(C const &c, F f, int, double, int)
        -> decltype(f(*std::begin(c), 0), std::declval<void>())
    {
        int counter{0};
        for (auto e : c)
        {
    ...
    template <typename C, typename F> // 1
    auto enumerate_helper(C const &c, F f, int, double, double)
        -> decltype(f(*std::begin(c)), std::declval<void>())
    {
        for (auto e : c)
            f(e);
    }
...
void enumerate(C const &c, F f)
{
    ns::enumerate_helper(c, f, 0, 0, 0);
};
