...
int main()
{
    ...
    std::ifstream f("first.txt");
    if(!f.is_open())
        {
            std::cerr<<"Cannot open file";
            return 0;
        }
    std::vector<Event> first_schedule{std::istream_iterator<Event>(f),
    ...
    std::ifstream s("second.txt");
    if(!s.is_open())
        {
            std::cerr<<"Cannot open file";
            return 0;
        }
    std::vector<Event> second_schedule{std::istream_iterator<Event>(s),
                                        std::istream_iterator<Event>()};
