...
namespace ns{
    ...
    auto enumerate_helper(C const& c, F f,int,int)
    ->decltype(c.size(), f(*std::begin(c),0,0), std::declval<void>())
    {
        int count = 0;
        for(auto e : c)
        {
    ...
    //2
    template <typename C, typename F>
    auto enumerate_helper(C const& c, F f ,int,double)
    ...
    //3
    template <typename C, typename F>
    auto enumerate_helper(C const& c, F f, double,double)
